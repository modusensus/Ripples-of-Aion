...
- LLM 精排接入检索出口：search 工具混合初排后由小型 LLM 重排（候选 ≤10、
  maxTokens 256、15s 超时）；索引强校验——越界/重复剔除、漏项按原相对序补尾，
  score 保持召回分；任何失败 log.warn 降级原序，绝不阻断检索。`rerankEnabled`
  默认开启（成本逃生门），面板设置页可关；hot-context 注入保持纯关键词不变
- 面板新增「检索台」页：与 search 工具同链路的真·混合检索（关键词+向量），
  命中行显示相关度分数、来源徽章（关键词/向量/混合）、热度徽章与实体色点；
  返回条数 5/10/20，精排开关可临时切换；面板检索不 bumpHeat，调试不污染热度
- 面板新增「图谱」页：实体共现力导向图（零依赖手写布局，同数据同形态）——
  节点半径随共现度、实体色点着色、边权定粗细与透明度；悬停高亮邻接，点击
  节点跳记忆页按实体过滤；实体不足 2 个显示空态
- 面板导航扩为五页：记忆 / 图谱 / 检索台 / 状态 / 设置；新增面板 IPC
  get-graph、search-memories；save-config 白名单加入 rerankEnabled

### 兼容性

- 纯增量改动：工具输出格式与存储数据格式不变；宿主 LLM 服务缺失时精排自动
  缺席（初排原序），面板检索台降级关键词来源
- 测试 94 → 112 用例（新增套件：精排 8、图谱 6；契约 +2、工具 +2）
