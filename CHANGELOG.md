...
- 面板交互式记忆浏览器（布局参考同作者的 dsh-mneme 记忆库）：顶部居中标签
  导航（记忆 / 实体 / 状态 / 设置）+ 卡片/时间线视图切换；记忆页三栏——
  **语义检索**（关键词防抖搜索、返回条数、时间范围）｜**过滤器**（热度
  热/温/冷三档、来源含已遗忘、实体 facet 计数色点）｜**时间树**（年/日
  两级折叠分组、实体色点着色、热度火焰徽章、行内遗忘）
- 图标素材：内联 Lucide v1.42 路径数据（morphicons 配套数据包，ISC
  license），沿用 dsh-mneme 的零依赖内联惯例，stroke 跟随主题色
- 「状态」页呈现 autoDream 记忆沉淀：主题簇（附成员原文）、矛盾标注（附
  双方原文）与上次做梦时间；「立即做梦」按钮手动触发一次整合——与空闲
  触发共用同一 single-flight 通道，做梦中显示呼吸动效并自动轮询结果
- 「设置」页：autoDream / 记忆热度 / 注入与向量检索的白名单配置项可在
  面板编辑，保存经类型校验合并写入（不再需要手工改存储键）
- 窗口默认尺寸 720×900 → 1560×1000（最小 1000×660），对齐交互式三栏布局

### 变更

- 滚动体验：外框与导航栏固定，三栏内容各自独立滚动（可见的粉色滚动条，
  支持滚轮与拖动）；时间树年/日表头滚动吸附，记忆多了也能定位到日期
- get-state 洞察摘要升级：主题簇附成员原文（≤8 条）、矛盾附双方原文，
  新增 dreaming 在途标志供面板动效与轮询
- 新增面板 IPC：browse-memories（关键词/全量 + 热度/实体/软删标记）、
  dream-now、get-config、save-config（保存按白名单键 + 类型校验合并，
  非法键静默丢弃）

### 兼容性

- 窗口尺寸持久化沿用既有机制，老用户保留已调整的布局；纯界面层改动，
  数据格式不变
- 测试 93 → 94 用例（契约 +1：dream-now 触发、6 通道注册与配置白名单合并）

## [0.4.2] - 2026-09-09

### 修复

- autoDream 空转：聚类与候选对只读顶层 `entities` 字段，而实测抽取器几乎只填
  `entityClaims`（本机 96 条活跃记录中 0 条带 entities、62 条带 claims）——共现
  图恒为空，每次整合都停在「没有可整合的主题簇」。现在实体来源取两者并集，
  存量记忆库无需重抽立即生效
- 枢纽实体阻尼：出现在 ≥50% 且 ≥8 条候选记录中的实体（如陪伴库里的「用户」）
  不再参与共现建图，避免所有记录经枢纽连成一团、聚类失去区分度；冲突候选对
  不受阻尼影响（广撒网 + 40 对上限控成本）。相对比例 + 绝对下限双条件，
  小样本/新库不误伤

### 兼容性

- 测试 89 → 93 用例（claims 聚类回归、枢纽阻尼、候选对 claims 命中 ×4）
